...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect {Dessert.new("cake", "lots", "tim")}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ...
      sample.each { |item| dessert.add_ingredient(item) }
      dessert.mix!
      expect(dessert.ingredients).not_to eq(sample)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      ...
      expect(dessert.serve).to eq("Chef Peter the Great Baker has made 10 cookies!")
    end
